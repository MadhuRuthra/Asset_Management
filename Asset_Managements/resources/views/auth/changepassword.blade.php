            ...
            <div class="col-md-6 p-0">
                <div class="card card-transparent auth-card shadow-none d-flex justify-content-center mb-0">
                    <div class="card-body">
                        ...
                        <h2 class="mb-2">Change Password</h2><br>

                        <x-auth-validation-errors class="mb-4" :errors="$errors" />
                        <form action="{{ route('changepasswordsave') }}" method="POST" name="new_entry_form" id="new_entry_form" onsubmit="" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="floating-label form-group">
                                        ...
                                        <div class="input-group">
                                            <input id="oldPassword" class="form-control" type="password" placeholder="Enter Your Old Password" name="old_password" required autocomplete="current-password" minlength="8" maxlength="15">
                                            <div class="input-group-append">
                                                <span class="input-group-text" id="toggleOldPassword" style="height:45px;">
                                                    <i class="fa fa-eye"></i>
                                                </span>
                                            </div>
                                        </div>
    ...
    <script>
        ...
        function validatePassword() {
            ...
            if (oldPassword.length < 8) {
                oldPasswordError.textContent = 'Old password is too short';
                valid = false;
            } else {
            ...
            submitButton.disabled = !valid || oldPassword.length === 0 || newPassword.length === 0 || confirmPassword.length === 0 || oldPassword.length > 8;
        }

        function validateOldPassword(oldPassword) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'validate-old-password', true); // Update the URL to your validation endpoint
            xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
            xhr.setRequestHeader('X-CSRF-TOKEN', '{{ csrf_token() }}'); // Include CSRF token for Laravel
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    if (!response.valid) {
                        ...
                        oldPasswordError.textContent = '';
                        validatePassword(); // Revalidate passwords on successful old password check
                    }
                }
            };
