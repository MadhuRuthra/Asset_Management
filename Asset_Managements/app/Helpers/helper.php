...
function activeRoute($routes, $isClass = false): string
{
    $currentRouteName = request()->route()->getName();

    // Ensure $routes is an array
    if (!is_array($routes)) {
        $routes = [$routes]; // Convert to array if a single string is provided
    }

    // Check if current route name matches any of the routes
    $isActive = in_array($currentRouteName, $routes);

    if ($isClass) {
        return $isActive ? ($isClass ?: 'active') : '';
    } else {
        return $isActive ? 'active' : ''; // Return 'active' if isActive is true
    }
}
